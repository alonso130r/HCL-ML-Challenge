#!/usr/bin/env python3
"""Select fusion learning rate by 3x3 CV, then train one final MELD model.

The frozen context-two BERT and audio caches must already exist. Three
dialogue-level folds over the development split compare three fusion learning
rates while retaining the full official training split. The winning rate is
retrained for the median selected epoch on train plus dev, then evaluated once
...
def fusion_arguments(args, learning_rate, output_dir, epochs=None, patience=None):
...
def run_cross_validation(records, args, device):
    ...
    for learning_rate in args.learning_rates:
        ...
        for fold in range(args.folds):
            ...
            run_dir = args.output_dir / "cv-dev-folds" / rate_name / f"fold-{fold + 1}"
            metrics_path = run_dir / "metrics.json"
            if metrics_path.exists() and not args.retrain:
                report = json.loads(metrics_path.read_text(encoding="utf-8"))
                print(f"Reusing {rate_name}, fold {fold + 1}", flush=True)
            else:
                print(f"Training {rate_name}, fold {fold + 1}/{args.folds}", flush=True)
                run_args = fusion_arguments(args, learning_rate, run_dir)
                report = stabilized.train(run_args, split_records, device, fold + 1)
            fold_reports.append(
                {
                    "fold": fold + 1,
                    "best_epoch": report["best_epoch"],
                    "weighted_f1": report["test_recurrent_matched"]["weighted_f1"],
        ...
        summary = {
            "learning_rate": learning_rate,
            "weighted_f1": float(np.mean([row["weighted_f1"] for row in fold_reports])),
            "macro_f1": float(np.mean([row["macro_f1"] for row in fold_reports])),
            "state_margin": float(np.mean([row["state_margin"] for row in fold_reports])),
            "zero_audio_margin": float(
                np.mean([row["zero_audio_margin"] for row in fold_reports])
            ),
            "audio_margin": float(np.mean([row["audio_margin"] for row in fold_reports])),
            "selected_epoch": selected_epoch([row["best_epoch"] for row in fold_reports]),
            "folds": fold_reports,
        }
    ...
    report = {
        "protocol": "full official train plus two development folds; validate on the third development fold",
        "folds": args.folds,
        "seed": args.seed,
    ...
    (args.output_dir / "cv_summary_dev_folds.json").write_text(
...
def train_final_model(records, args, selected, device):
    ...
    final_dir = args.output_dir / "best-model-dev-cv"
...
def validate_arguments(args):
    if args.folds != 3 or len(args.learning_rates) != 3:
        raise ValueError("the bounded final protocol requires three folds and three rates")
